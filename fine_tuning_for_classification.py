...
#Set up pytorch dataloaders that will be used to train the model
import tiktoken

tokenizer = tiktoken.get_encoding("gpt2")
print(tokenizer.encode("<|endoftext|>", allowed_special={"<|endoftext|>"}))

#Implement a pytorch dataset which specifies how the data is loaded and processed
#before we can instantiate the data loaders
import torch
from torch.utils.data import Dataset

#encodes the text messages into token sequences, identifies the longest sequence in the training dataset, adds padding
class SpamDataset(Dataset):
    def __init__(self, csv_file, tokenizer, max_length=None, pad_token_id=50256):
        self.data = pd.read_csv(csv_file)

        self.encoded_texts = [tokenizer.encode(text) for text in self.data["Text"]] # Pre-tokenize texts
...
print(train_dataset.max_length) #to see the number of tokens in the longest sequence   

#Pad the val and test sets to match the length of the longest training sequence
...
#Creating the pytorch data loaders (create the training, val and test set data loaders 
# that loads the text messages and labels in batches of size 8) 
...
#total number of batches in each dataset
...
from training_an_llm import load_weights_into_gpt
...
#to ensure model was loaded correctly, double check if it produces coherent text
...
#before starting to fine-tune the model as a spam classifier, check if the model already classifies spam
#messages by prompting it with instructions
text_2 = (
...
#to make the ready for classification fine-tuning, first freze the model, meaning making all layers non-trainable
...
#to make the final layernorm and last transformer block trainable, set their requires_grad to true
...
#since using softmax is optional becasue the largest outputs directly correspond to the highest probability,
...
#to compute the loss for a single batch obtained from the previously defined data loaders
...
#to calculate the loss for all batches in a data loader, define the calc_loss_loader as before
...
#compute the inital loss for each data set
...
#initialize the optimizer, set the number of training epochs, initiate the training
import time
...
#once saved the model could be loaded as follows in a new session
